...
public class ShipController : Status
{
    ...
    void Awake()
    {
        SetCurrentSecondaryCannon(settings.secondaryCannon);
        myType = settings.myType;
        myDroneType = settings.myDroneType;
        bullet = settings.bullet;
        currentSpecialAble = settings.mySpecial;

        healthLevel = settings.healthLevel;
